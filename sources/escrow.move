...
use escrow::escrow_utils::{
    get_coin_from_dof,
    add_items_to_dof,
    get_items_from_dof,
    transfer_items,
    check_items_ids,
    check_vector_for_duplicates
};
use std::string::utf8;
...
entry fun create<T: key, N: key + store>(
    ...
    let has_recipient_assets =
        vector::length(&recipient_items_ids) > 0 || recipient_coin_amount > 0;
...
entry fun cancel<T: key, N: key + store>(escrow: Escrow<T, N>, clock: &Clock, ctx: &TxContext) {
